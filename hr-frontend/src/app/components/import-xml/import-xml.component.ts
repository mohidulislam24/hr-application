import { Component } from '@angular/core';
import { ImportService } from '../../services/import.service';
import { Employee } from '../../models/employee.model';

@Component({
  selector: 'app-import-xml',
  templateUrl: './import-xml.component.html',
  styleUrls: ['./import-xml.component.css']
})
export class ImportXmlComponent {
  xmlData: string = `<?xml version="1.0" encoding="utf-16"?>
<employees>
    <employee id="129">
        <firstname>Jane</firstname>
        <lastname>Doe</lastname>
        <title>Engineer</title>
        <division>Materials</division>
        <building>327</building>
        <room>19</room>
    </employee>
    <employee id="130">
        <firstname>William</firstname>
        <lastname>Defoe</lastname>
        <title>Accountant</title>
        <division>Accts Payable</division>
        <building>326</building>
        <room>14a</room>
    </employee>
    <employee id="131">
        <firstname>Jack</firstname>
        <lastname>Dee</lastname>
        <title>Engineering Manager</title>
        <division>Materials</division>
        <building>327</building>
        <room>21</room>
    </employee>
    <employee id="132">
        <firstname>Sandra</firstname>
        <lastname>Rogers</lastname>
        <title>Engineering</title>
        <division>Materials</division>
        <building>327</building>
        <room>22</room>
    </employee>
    <employee id="133">
        <firstname>Steve</firstname>
        <lastname>Casey</lastname>
        <title>Engineering</title>
        <division>Materials</division>
        <building>327</building>
        <room>24</room>
    </employee>
    <employee id="135">
        <firstname>Michelle</firstname>
        <lastname>Michaels</lastname>
        <title>COO</title>
        <division>Management</division>
        <building>216</building>
        <room>264</room>
    </employee>
</employees>`;
  importedEmployees: Employee[] = [];
  isLoading = false;
  successMessage = '';
  errorMessage = '';

  constructor(private importService: ImportService) { }

  importXml(): void {
    this.isLoading = true;
    this.successMessage = '';
    this.errorMessage = '';

    this.importService.importXml(this.xmlData).subscribe(
      (employees) => {
        this.importedEmployees = employees;
        this.isLoading = false;
        this.successMessage = `Successfully imported ${employees.length} employees!`;
      },
      (error) => {
        this.isLoading = false;
        this.errorMessage = 'Error importing XML data. Please check the console for details.';
        console.error('Error importing XML', error);
      }
    );
  }
}
